...
int main( int argc, char* argv[] )
{
    ...
    // Get a context
    rc = PWR_CntxtInit( PWR_CNTXT_DEFAULT, PWR_ROLE_APP, "App", &cntxt ); //&cntxt is an opaque pointer that contains data for system description
    assert( PWR_RET_SUCCESS == rc );

    rc = PWR_CntxtGetEntryPoint( cntxt, &self ); //sets PWR_Obj at some point in the system description (in this case a node)
    assert( PWR_RET_SUCCESS == rc );

	PWR_ObjType objType;
	PWR_ObjGetType( self, &objType ); //gets the object type of self
    printf("I am a `%s`\n", PWR_ObjGetTypeString( objType ) );

    PWR_Obj parent;

	rc = PWR_ObjGetParent( self, &parent );
  printf("parent rc is %i\n",rc); //returned 2
	assert( rc >= PWR_RET_SUCCESS );


    PWR_Grp children;
	rc = PWR_ObjGetChildren( self, &children );
  printf("children rc is %i\n",rc); //returned 2
	assert( rc >= PWR_RET_SUCCESS );
    int i;
